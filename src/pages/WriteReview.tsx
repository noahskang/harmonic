...
export default function WriteReview() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!data) { navigate('/pending-reviews'); return }
      setRequest(data)
      setLoading(false)
    }
    load()
  ...
  if (loading || !request) return <Layout><p className="text-stone-400 text-sm">Loading...</p></Layout>

  return (
    <Layout>
      <div className="max-w-2xl">
        <div className="mb-8">
          <p className="text-sm text-stone-400 mb-1">Writing feedback for</p>
          <h1 className="text-2xl font-semibold text-stone-700">{request.requester_name}</h1>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="px-5 py-2.5 border border-stone-200 text-stone-500 rounded-xl text-sm hover:bg-stone-50 transition-colors"
              >
                Cancel
              </button>
            </div>
